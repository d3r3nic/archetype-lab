...
Downstream adoption, reported by the game session on 2026-09-11 and checked read-only from the factory session: the game repository (branch archetype-bootstrap, previously at 326a030) ran its installed update.sh twice; the session reported 54 managed files updated, 3 added (the validator and its two data files), root CLAUDE.md updated, References.md and feature-tree.md untouched, the seven new dials recorded in the game's References.md, and the result committed as 42592a7. Observed from the factory session on the same day: that commit is the branch head with a clean tree, VERSION-LOG.md records 5c305ef, and the installed validate-framework.sh reports group 10 clean with 0 errors and 0 warnings. The game session's original trigger pattern now finds only false positives on "Expo" inside "Exposing" and "Export".
...
4. Updater no-op entries. Observed in the game repository after two consecutive runs of the installed update.sh at the same revision: VERSION-LOG.md carries two identical 5c305ef entries, and the "(none yet ...)" placeholder line above them was never removed. Harmless, but the updater should skip the log entry when nothing changed and drop the placeholder on first update, with an idempotent-second-run regression in scripts/test-entrypoints.py. Belongs to the updater's own step, not this one.
