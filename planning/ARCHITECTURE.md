...
## Decision authority and the operating profile (Step 64, designed)

Two project-owned facts every session reads before acting. The decision-authority setting says who decides technical questions: under `ai-decides` the AI decides, records the decision with its reason and the alternatives rejected, and escalates only the four constitution categories (new recurring spend, external commitments, actions on a live customer environment, a change to what the product is) as one plain recommendation; under `owner-decides` convention 19's approval rules apply as written. The operating profile (PROFILE.md at the project root, bullets parsed like References.md) records the stage (`isolated`, `trial`, `operational`) the AI derived from recorded facts (audience, real personal data, external effects, operational reliance, contributors, obligations, cost ceilings), with `unknown` explicit and never read as "no". A floor of six obligations applies at every stage whenever the protected asset exists. Deferrals live in TECHNICAL-DEBT.md with a control, a named trigger or date, and closure evidence; a trigger the facts make true turns the deferral blocking. scripts/validate-profile.sh reports PASS, FAIL, WARN, DEFERRED, or UNVERIFIED per check and treats an absent profile as the strictest reading with a warning. Design: planning/STEP-64-DECISION-AUTHORITY-AND-PROFILE.md.
