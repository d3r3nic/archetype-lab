...
The founder restated the goal on 2026-09-12: a harness a non-technical owner can ride, asked only about scope, the idea, and the costs that matter, with the AI deciding and recording everything technical; rigor from a disposable experiment to an operational product, with deferrals planned rather than forgotten. Five Astra research runs (research/astra-2026-09-12/) and the factory session's own assessment agree on the first two foundations. Decision: a declared decision-authority setting (owner-decides or ai-decides) carries the engineering constitution into every project and reconciles convention 19 and the enforcer with it; an operating profile (stage derived from recorded facts, a floor no stage may defer, trigger-linked deferrals in the debt log) is the record every later step reads. Design and acceptance criteria in [STEP-64-DECISION-AUTHORITY-AND-PROFILE.md](STEP-64-DECISION-AUTHORITY-AND-PROFILE.md). Steps 65 to 68 are planned from the same research.
