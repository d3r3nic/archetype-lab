...
Measured on 2026-09-11 in the factory dist checkout: 307 findings before (234 names, 4 step references, 8 AI statistics, 8 numeric limits, 16 changelog lines, 37 missing notices) across 48 files; 0 after. Planted violations in every class fail the validator (output in the step record). validate-framework.sh passes with 0 errors and 0 warnings; test-entrypoints.py passes 14 regressions including the two-step upgrade from the pre-AGENTS release 46ebfa4. Phase playbooks remain outside the validator's default scope (bootstrap/ 77, scaffolding/ 96, development/ 50 findings measured) and are recorded as follow-up. Product publication and the independent audit verdict are recorded in the evidence update below this entry once observed.
