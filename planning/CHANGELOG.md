...
## 2026-09-12 (Step 64) — Decision authority and the operating profile

Trigger: the founder restated the goal of the framework (a harness a non-technical owner can ride, asked only about scope, the idea, and the costs that matter, with the AI deciding and recording everything technical; rigor from a disposable experiment to an operational product with deferrals planned, not forgotten). Five Astra research runs and the factory session's assessment agreed on two foundations, and on a contradiction to fix first: convention 19 and the enforcer still handed architecture and approval to the human.

Changes in dist/: conventions/29-decision-authority.md (owner decides the product, AI decides and records everything consequential, five escalation categories, one recommendation, no unverified claims, done means right, independent review, owner safety rails; setting owner-decides or ai-decides with its source) and conventions/30-operating-profile.md (PROFILE.md with an operating stage derived from facts with explicit unknown, a six-item floor never deferred, deferrals in TECHNICAL-DEBT.md with control, trigger or inclusive date, review date, closure evidence); conventions/19-steering.md reconciled to the setting; CLAUDE.md and Conventions.md route both and state no counts; templates/profile.md new, templates/technical-debt.md with the deferral fields and the exact-label rule, references templates with profile, owner channel, decision location, and reporting pace, feature-tree with the deferred status; bootstrap/ONBOARD.md with discovery Group 6, one recommendation instead of a menu, profile generation on every path, and the log; EXISTING-PROJECT.md, RED-FLAGS.md, scaffolding/_preamble.md and the scaffold playbooks, development/DEVELOP.md and MAINTAIN.md, README.md; scripts/validate-profile.sh (OK, FAIL, WARN, DEFERRED, UNVERIFIED; --strict; a missing profile is the strictest reading and cannot hide deferrals; a floor item is never postponed; the documented entry format is the contract and every deviation the parser can recognize fails, while any other line mentioning a field name warns), scripts/test-profile.py (157 tests), validate-framework.sh group 12 (vocabulary and key set agree across validator, templates, and convention) and group 2 (no stated count is fine).

Method: design record first (planning/STEP-64-DECISION-AUTHORITY-AND-PROFILE.md), reviewed adversarially by the founder's Codex CLI before implementation (FIX-FIRST, ten findings, all folded in). Implementation by the factory session. Independent audit of the product branch by an Opus agent (twenty-eight reports, APPROVE on the final revision) and the Codex reviewer (twenty-seven rounds, all findings fixed through round twenty-six, the rest recorded as the parser's stated limit; the factory session closed the loop when the founder asked whether the work was circling). Published as archetype PR #4, revision 0e4b5f6a550ad163fbac33037734bf7f742ff335. Details, proofs, and the audit trail in the step record.
...
Method: validators and fixes by the factory session; playbooks remediated by three Opus agents in parallel, one per directory, every diff line reviewed by the factory session before commit. Verified locally: timeless and self-claims checks clean, validate-framework.sh 0 errors and 0 warnings, 14 distribution regressions OK with the 46ebfa4 legacy export. Independent Opus audit: FIX-FIRST with 12 findings and no lost force in the playbooks, closed in two fix commits, then APPROVE. Published 2026-09-11 in [archetype PR #3](https://github.com/d3r3nic/archetype/pull/3), revision 3e670a57118a9127f0e726b1cb6c5cd97a228120; on the merged main validate-framework.sh reports 0 errors and the 14 regressions pass. Factory design PR #7 (d69ab27) preceded it. The game session reported adopting 3e670a5 the same day through one run of its installed update.sh (its PR #2, 8a515fb); the factory session observed the clean master head, the single VERSION-LOG entry, and the installed self-test passing with groups 10 and 11 clean. Other installations are not updated by this step.
...
Changes in dist/: new scripts/validate-timeless.sh with scripts/timeless-terms.txt and scripts/timeless-allowlist.txt, wired into scripts/validate-framework.sh as group 10; 29 universal and 7 backend conventions remediated (names out of rule zones, no factory step references, no AI statistics, tool-bound limits rewritten as project-set dials, changelog language removed, every Research Notes section opens with the dated notice); #22 reworded to "prefer an established foundation when one fits; a project may rule otherwise and records it in References.md"; #26 and #28 gained "Applies when"; #8 gained the transpiled-error rule that CLAUDE.md carried as a code snippet; CLAUDE.md, Conventions.md, backend/Conventions.md, README.md, and every file template remediated; pulse implementation notes moved from #26 to templates/pulse-monitor-spec.md; hooks-spec host contract marked dated with the one allowlisted host name.

Measured on 2026-09-11 in the factory dist checkout with the final validator: 312 findings before (238 names, 4 step references, 8 AI statistics, 9 numeric limits, 16 changelog lines, 37 missing notices) across 48 files; 0 after. Planted violations in every class fail the validator (output in the step record). validate-framework.sh passes with 0 errors and 0 warnings; test-entrypoints.py passes 14 regressions including the two-step upgrade from the pre-AGENTS release 46ebfa4. An independent Opus audit returned FIX-FIRST with 13 findings; all but one were closed in a fifth product commit and one was declined on the founder's recorded steering (Research Notes name categories); the step record has the detail. Phase playbooks remain outside the validator's default scope (bootstrap/ 73, scaffolding/ 101, development/ 47 findings measured) and are recorded as follow-up. Published 2026-09-11 in [archetype PR #2](https://github.com/d3r3nic/archetype/pull/2), revision 5c305ef590cc2787b9c3aaa81a2c7b6aff1e0d67, after the same auditor re-reviewed both fix commits and returned APPROVE. Factory design PR #3 (e00981d) preceded it. On the merged product main, validate-framework.sh reports 0 errors and 0 warnings and the 14 regressions pass. Downstream installations pull the revision through their installed updater; none were updated by this step. The game session reported adopting 5c305ef the same day via two runs of its installed update.sh (commit 42592a7 in its repository); the factory session observed the clean branch head, the VERSION-LOG entry, and a passing installed self-test, and logged a follow-up: the updater appends a duplicate VERSION-LOG entry on a no-op second run and leaves the placeholder line in place.

## 2026-09-10 (Step 61) — Shared task and freshness rulebook

Trigger: an existing project needs centrally maintained task behavior and a project-local platform binding. Design recorded in [STEP-61-TASK-RULEBOOK.md](STEP-61-TASK-RULEBOOK.md) and published first in factory PR #1. The reviewed product change was then published in [archetype PR #1](https://github.com/d3r3nic/archetype/pull/1), revision 326a0306949013b00fd6b3dca4375d78a76e8092. It adds managed AGENTS/CLAUDE entry points, safe destination/root checks, project-owned migration routing, and shared task/freshness protocols. All 14 regressions pass, including previous-release upgrade; source and injected self-tests have zero errors/warnings. An independent reviewer approved implementation 6c2445e before merge. Installed files and original-guidance preservation were verified byte-for-byte. Current updates remain manual and copy-based; runtime enforcement and automatic immutable rollout remain separate work. Keep generic rules public and private project mappings downstream. No runtime freshness guarantee is implied by instruction files.
