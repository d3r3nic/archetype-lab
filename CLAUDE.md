...
5b. **Framework encodes character; project artifacts hold specifics.** The dist/ framework must not contain specific tool names as THE answer, specific API calls, version numbers, current pricing, or vendor product names as prescriptions. Those are specifics — they belong in bootstrapped project artifacts (References.md, feature-tree.md, conventions/overrides/, docs/features/, CLAUDE.md.additions), which are expirable and updated per project. Test: "if rolling back a specific tool mention leaves the direction UNCHANGED, it was a signal — keep it. If the AI now doesn't know what to do, it was a snippet — promote the intent, drop the snippet." Tools are the developer's toolbox; the framework teaches character. Enforced since Step 62 by dist/scripts/validate-timeless.sh (run through validate-framework.sh): names only under "Research Notes" with the dated notice, no factory step references, no AI statistics, no tool-bound numeric limits, no changelog language. Add a term to scripts/timeless-terms.txt when review finds a name the list missed; an allowlist entry needs a written reason. Since Step 63 the gate covers the phase playbooks too, a line beginning "Dated example:" is the one per-line allowed zone, and dist/scripts/validate-claims.sh (group 11) fails on any count, engine path, or convention reference the tree does not bear out: never state how many conventions or red flags exist, name them.
